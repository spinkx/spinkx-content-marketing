...
function spinkx_cont_widget_css_js($path) {
	...
	.se-pre-con { position: fixed; left: 0; top: 0; width: 100%; height: 100%; z-index: 9999;	
	 background: url( ' . esc_url( $path[0] ) . 'images/loader.gif) center no-repeat rgba(0, 0, 0, 0.2); }
...
function spinkx_cont_cp_css_js( $callFrom = null, $path ) {
	...
	.js #loader { display: block; position: absolute; left: 100px; top: 0; }
	.se-pre-con { position: fixed; left: 0; top: 0; width: 100%; height: 100%; z-index: 9999; 
	background: url( ' . $path[0]  . 'images/loader.gif) center no-repeat rgba(0, 0, 0, 0.2);; }
...
function spinkx_cont_campaign_set_css_js($path) {
	...
	.se-pre-con { position: fixed; left: 0px; top: 0px; width: 100%; height: 100%; z-index: 9999;
	 background: url(' . $path[0] . 'images/loader.gif) center no-repeat #fff; }
...
function spinkx_header_menu() {
    ...
    ?><div><div class="spnx-menu-logo"><span><a href="https://www.spinkx.com"><img src="<?php echo SPINKX_CONTENT_CDN_URL; ?>media/spinkx-logo.png" /></a></span></div>
	...
	if('spinkx_content_play_list' === $page ) {
		...
		if( $cp_noitce_info ) {
			...
			echo '<a href="javascript:;void(0)" class="spnx-faq" onclick="showNoticeCP();" style="margin-top: 10px; margin-left: 500px;"><i>Boost Post FAQ\'s</i></a>';
			$info_flag = true;
		}
		...
		if(  $campaigns_noitce_info ) {
			...
			echo '<a href="javascript:;void(0)" class="spnx-faq" onclick="showNoticeCampaign();" style="margin-top: 10px; margin-left: 655px;"><i>Campaign FAQ\'s</i></a>';
			$info_flag = true;
		}
	 }
	 if( $noitce_flag ) {
